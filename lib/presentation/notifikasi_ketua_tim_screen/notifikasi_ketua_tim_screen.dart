...
import '../dashboard_ketua_tim_screen/dashboard_ketua_tim_screen.dart';
...
class NotifikasiKetuaTimScreen extends StatefulWidget {
  const NotifikasiKetuaTimScreen({Key? key}) : super(key: key);

  @override
  _NotifikasiKetuaTimScreenState createState() => _NotifikasiKetuaTimScreenState();
}

class _NotifikasiKetuaTimScreenState extends State<NotifikasiKetuaTimScreen> {
    ...
    try {
      ...
      if (user != null) {
        ...
        if (userDoc.exists) {
          ...
          final currentTeamId = userData['team_id']?.toString() ?? '';

          setState(() {
            teamId = currentTeamId;
          });

          // Ambil data notifikasi berdasarkan `team_id`
            ...
            for (var submission in submissionsSnapshot.docs) {
              ...
              switch (rawStatus) {
                case 'PROCESSING':
                  ...
                  break;
                case 'TEAM_REJECTED':
                  statusText = 'Ditolak oleh Ketua Tim';
                  break;
                case 'TEAM_APPROVED':
                  statusText = 'Menunggu Persetujuan Pimpinan';
                  break;
                case 'HEAD_REJECTED':
                  statusText = 'Ditolak oleh Pimpinan';
                  break;
                case 'HEAD_APPROVED':
                  statusText = 'Disetujui oleh Pimpinan';
                  break;
                default:
                  statusText = '';
              }
      ...
      itemBuilder: (context, index) {
        ...
        switch (notification['Status']) {
          ...
          case 'Menunggu Persetujuan Pimpinan':
            statusBackgroundColor = Colors.blue.shade400;
            break;
          ...
          onTapRow: () {
            ...
            if (submissionType == "Presensi Manual" && notification['Status'] == 'Menunggu Persetujuan') {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => DetailAbsensiManualKetuaTimScreen(
                    data: notification,
                  ),
                ),
              );
              ...
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => DetailPengajuanCutiKetuaTimScreen(
                    data: notification,
                  ),
                ),
              );
